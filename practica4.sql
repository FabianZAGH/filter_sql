...
por el número de empleado, nombre, departamento de todos los empleados de la tabla empleados*/
...
número de departamentos por región y los imprima en pantalla*/
...
BEGIN
   FOR reg IN (SELECT DISTINCT region_id FROM departments) LOOP
      ...
      BEGIN
         -- Contar la cantidad de departamentos por región
         SELECT COUNT(*) INTO v_cantidad_departamentos
         FROM departments
         WHERE region_id = reg.region_id;
         DBMS_OUTPUT.PUT_LINE('Región ' || TO_CHAR(reg.region_id) || ': ' || v_cantidad_departamentos || ' departamentos');
      END;
   END LOOP;
END contar_departamentos_por_region;


/*Enunciado 9: Desarrolle unas función que devuelva la cantidad de empleados 
del departamento que se envíe por parámetro*/
...
/*Enunciado 10: Crear un procedimiento almacenado que simule una tabla de multiplicar en sentido inverso. El procedimiento recibe por parámetro del multiplicador*/
